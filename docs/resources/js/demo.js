type InputType = "string" | "local" | "remote" | "json";
...
interface Config {
    ...
    error: (err: any, file?: any) => void;
    download: boolean;
}

let inputType: InputType = "string";
let stepped = 0, rowCount = 0, errorCount = 0, firstError: any;
...
$(function() {
    ...
    $('#submit').click(function() {
        ...
        let input = $('#input').val() as string;

        if (inputType == "remote")
            input = $('#url').val() as string;
        else if (inputType == "json")
            input = $('#json').val() as string;
        ...
        if (inputType == "local") {
            ...
            $('#files').parse({
                config: config,
                before: function(file: File, inputElem: HTMLElement) {
                    start = now();
                    console.log("Parsing file...", file);
                },
                error: function(err: any, file: File) {
            ...
            start = now();
            const csv = Papa.unparse(input, config);
            end = now();
...
function printStats(msg: string) {
    if (msg)
        console.log(msg);
    console.log("       Time:", (end - start || "(Unknown; your browser does not support the Performance API)"), "ms");
...
function buildConfig(): Config {
    return {
        delimiter: $('#delimiter').val() as string,
        header: $('#header').prop('checked'),
        dynamicTyping: $('#dynamicTyping').prop('checked'),
        skipEmptyLines: $('#skipEmptyLines').prop('checked'),
        preview: parseInt($('#preview').val() as string || "0"),
        step: $('#stream').prop('checked') ? stepFn : undefined,
        encoding: $('#encoding').val() as string,
        worker: $('#worker').prop('checked'),
        comments: $('#comments').val() as string,
        complete: completeFn,
        error: errorFn,
...
function stepFn(results: any, parser: any) {
    stepped++;
    if (results) {
        if (results.data)
...
function completeFn(results: any) {
    end = now();
...
function errorFn(err: any, file?: any) {
    end = now();
    console.log("ERROR:", err, file);
    enableButton();
}
